...
describe('Courses Flow', () => {
    ...
    describe('Home Screen', () => {
        ...
        it('should be able to swipe banners', async () => {
            await waitFor(element(by.id('banners-carousel'))).toBeVisible().withTimeout(10000);
        ...
        it('should navigate to course from home', async () => {
            await waitFor(element(by.id('course-card-0'))).toBeVisible().withTimeout(5000);
            await element(by.id('course-card-0')).tap();
            await waitFor(element(by.id('course-detail-screen'))).toBeVisible().withTimeout(5000);
        });
    });
    ...
    describe('Courses Screen', () => {
        ...
        it('should filter courses by category', async () => {
            await waitFor(element(by.id('category-chip-0'))).toBeVisible().withTimeout(5000);
            await element(by.id('category-chip-0')).tap();
            await waitFor(element(by.id('courses-list'))).toBeVisible().withTimeout(5000);
        });

        it('should switch between categories', async () => {
            await waitFor(element(by.id('category-chip-0'))).toBeVisible().withTimeout(5000);
            await element(by.id('category-chip-0')).tap();
            await waitFor(element(by.id('courses-list'))).toBeVisible().withTimeout(3000);
    ...
    describe('Course Detail Screen', () => {
        ...
        it('should display modules if available', async () => {
            try {
                ...
                await waitFor(element(by.id('modules-list'))).toBeVisible().withTimeout(3000);
            } catch (e) {
